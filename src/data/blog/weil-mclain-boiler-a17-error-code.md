---
title: "Weil-McLain A17 Error Code - Causes & Fix"
description: "A17 is not a documented Weil-McLain code. Check for misread display (17, A1, or other brand) and consult your manual for the actual code."
pubDatetime: 2026-06-13T12:56:55Z
modDatetime: 2026-06-13T12:56:55Z
...
tags:
  - boiler
  - weil-mclain
money_part: "Weil-McLain Boiler Control Board"
diy_or_pro: "pro"
free_checks:
  - "Check the display in bright lighting to confirm the exact code-look for numeric codes like 17, 10, or 11 that may appear similar to A17."
  - "Power-cycle the boiler by turning off the service switch for 30 seconds, then back on, to clear any transient display glitches."
...
1. **Confirm the exact code** by viewing the boiler display in bright, direct lighting. Note every character, including leading zeros, letters, and any flashing or alternating segments.
2. **Power-cycle the boiler** by turning the service switch to OFF, waiting 30 seconds, then turning it back to ON. Observe whether the code clears, changes, or persists.
3. **Locate your boiler model and serial number** on the rating plate (usually on the jacket or inside the front panel) and confirm the brand is Weil-McLain.
4. **Retrieve the installation and operation manual** for your specific model from the Weil-McLain website or your installer's documentation, and turn to the troubleshooting or error code section.
5. **Match the displayed code** to the official chart. If the code does not appear, check for numeric codes that are visually similar (1, 7, 10, 11, 17, 70, 71, etc.).
6. **Inspect other displays** in the system, including zone controllers, aquastats, and thermostats, to determine if the A17 code is coming from an accessory rather than the boiler itself.
7. **Test the control board display** by entering diagnostic mode (if your model supports it) and observing whether the display shows valid test codes or continues to show invalid characters.
8. **Call a qualified boiler technician** if the code cannot be matched to any documented Weil-McLain error, if the control board appears to be malfunctioning, or if you are uncertain about the boiler model or brand.

## Parts Often Needed

| Part | Notes |
|------|-------|
| Weil-McLain Boiler Control Board | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-weil-mclain-boiler-a17-error-code&k=Weil-McLain+Boiler+Control+Board&tag=errorcodefixes-20) \| Only if the board is confirmed faulty and displays invalid codes after all sensor and wiring checks. Match the exact model number and firmware revision. |
| Boiler Pressure Sensor or Transducer | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-weil-mclain-boiler-a17-error-code&k=Boiler+Pressure+Sensor+or+Transducer&tag=errorcodefixes-20) \| Common replacement if the actual code is numeric 17 or similar and relates to pressure faults. Verify the correct part number for your model. |
