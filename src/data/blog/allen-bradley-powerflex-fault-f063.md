...
| Part | Notes |
|------|-------|
| Motor bearings | Replace if driven equipment has a seized bearing causing overload |
| Motor (larger frame) | Required if motor is confirmed undersized for the application |
| PowerFlex control board | Only if fault persists with correct parameters and no mechanical cause |
