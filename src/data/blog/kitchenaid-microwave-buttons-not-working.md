---
title: "KitchenAid Microwave Buttons Not Working - Causes & Fix"
description: "Usually Control Lock is enabled or the control board needs a power reset. Turn off the breaker for 1 minute, then disable Control Lock."
pubDatetime: 2026-06-11T11:35:30Z
modDatetime: 2026-06-11T11:35:30Z
...
1. Turn off the circuit breaker feeding the microwave and leave it off for 1 minute to reset the control boards, then restore power and test all buttons.
2. Check whether Control Lock is enabled (consult your model's user manual for the specific button combination to disable it, often holding a button for 3 seconds).
3. Unplug the microwave or switch off the breaker again before accessing any internal components.
4. Remove the outer cabinet or control panel cover to expose the touchpad and control board assembly.
5. Inspect the ribbon cable connector between the touchpad and control board for looseness, corrosion, or damage, and reseat the connection if it looks questionable.
6. Test button behavior carefully: if no buttons respond at all, suspect the user control board or main control board. If only some buttons fail, the touchpad membrane is likely worn out.
7. Replace the failed component (touchpad if partial failure, user control or main control board if total failure) and reassemble the unit.
8. Restore power and verify that all buttons now respond correctly before returning the microwave to service.

## Parts You Might Need

| Part | Notes |
|------|-------|
| Touchpad / Control Panel | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-kitchenaid-microwave-buttons-not-working&k=Touchpad+%2F+Control+Panel&tag=errorcodefixes-20) \| Replace when only some buttons fail to respond. |
| User Control and Display Board | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-kitchenaid-microwave-buttons-not-working&k=User+Control+and+Display+Board&tag=errorcodefixes-20) \| Replace when all buttons are dead and the display is also affected. |
| Main Control Board | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-kitchenaid-microwave-buttons-not-working&k=Main+Control+Board&tag=errorcodefixes-20) \| Replace when the entire control system is unresponsive and user board tests good. |
