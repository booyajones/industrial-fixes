---
title: "Ice Machine Error Code Lookup: All Brands"
description: "Master ice machine error code lookup guide covering Hoshizaki, Manitowoc, Scotsman, Ice-O-Matic, Follett, and more."
pubDatetime: 2026-04-22T23:00:00Z
modDatetime: 2026-04-22T23:00:00Z
author: "ErrorCodeFixes"
featured: true
draft: false
tags:
  - refrigeration
  - ice-machine
  - lookup
---

## Ice Machine Error Code Lookup — Start Here

Ice machine codes usually point to freeze time, harvest time, water level, condenser problems, sensor faults, or bin controls. This page is the high-level lookup hub for the major commercial ice machine brands and the fault patterns technicians see most often.

[Jump to Fix](#fix)

## Common Ice Machine Fault Patterns

| Fault Type | What It Usually Means |
|---|---|
| Long freeze | Scale, dirty condenser, low charge, warm water |
| Long harvest | Hot gas issue, curtain/gate problem, slab release issue |
| High pressure | Condenser dirty or fan/water regulation problem |
| Water fault | Float switch, inlet valve, low pressure |
| Sensor fault | Thermistor or float reading out of range |
| Bin full / dispenser fault | Bin switch, auger, gate, or dispenser jam |

## Brand Hubs Covered

- Hoshizaki
- Manitowoc
- Scotsman
- Ice-O-Matic
- Follett
- Welbilt / Manitowoc variants

## Step-by-Step Fix {#fix}

1. **Check whether the machine is a cube, flaker, nugget, or dispenser model**.
2. **Clean first if maintenance is overdue** — Scale causes many false fault patterns.
3. **Inspect water supply, drain, and condenser condition**.
4. **Watch whether the fault appears in freeze, harvest, or dispense**.
5. **Use the model-specific guide once you know the stage of failure**.

## Parts Often Needed

| Part | Notes |
|---|---|
| Water inlet valve | Fill issues |
| Float switch | Water level faults |
| Thermistor | Freeze/harvest sensor problems |
| Hot gas valve | Harvest faults |
| Condenser fan motor | High-pressure trips |

## When to Call a Pro

If the machine is in a foodservice or healthcare setting and codes continue after cleaning and basic checks, bring in a commercial refrigeration tech. Ice quality, sanitation, and production rate all matter, not just whether the machine turns on.
