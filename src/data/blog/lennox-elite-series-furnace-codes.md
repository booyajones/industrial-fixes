...
| Part | Notes |
|---|---|
| SureLight igniter | Silicon nitride; part number varies by model |
| Flame sensor | Clean with fine steel wool; measure µA output |
| Pressure switch | Single or dual switch depending on model |
| ECM blower motor module | Replace module only if motor spins freely |
| High-limit switch | Check reset button — some are auto-reset, some manual |
| Rollout switch | Manual-reset; do not bypass |
