...
description: "F24 is a service error on Amana dryers. Reset power at the breaker for 5 minutes, then run a timed dry cycle to clear the fault."
pubDatetime: 2026-05-31T04:30:05Z
modDatetime: 2026-05-31T04:30:05Z
...
1. Turn off power at the circuit breaker that supplies the dryer, not just the control dial, and wait a full five minutes to allow the control board to fully discharge and clear stored faults.
2. Restore power at the breaker and confirm the dryer control panel lights up normally without displaying the F24 code on startup.
3. Select a Time Dry cycle on the control panel, set any heat and time settings, and press start to begin the cycle.
4. Observe the dryer for at least one full minute while it runs to confirm that the F24 code does not reappear on the display.
5. If the code does not return, allow the dryer to complete a full cycle and resume normal use, as the fault was transient and has cleared.
6. If the F24 code returns immediately or within the first minute of operation, turn off the dryer and locate the model and serial number tag on the inside of the door opening or on the rear cabinet.
7. Contact Amana service or consult the model-specific technical service sheet to identify the exact component or sensor associated with F24 for your dryer model, since the public guidance does not name a replacement part.

## Parts Often Needed

| Part | Notes |
|------|-------|
| Main electronic control board | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-amana-dryer-f24-error-code&k=Main+electronic+control+board&tag=errorcodefixes-20) \| Only if the tech sheet confirms the control itself has failed and the code returns after reset. |
| Wire harness or connector kit | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-amana-dryer-f24-error-code&k=Wire+harness+or+connector+kit&tag=errorcodefixes-20) \| If inspection reveals a damaged or corroded connection to the control assembly. |
