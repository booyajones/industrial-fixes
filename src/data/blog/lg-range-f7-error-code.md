---
title: "LG Range F7 Error Code - Causes & Fix"
description: "F7 means the oven failed to heat above 150°F during preheat. Most often caused by a failed upper convection fan or faulty temperature sensor."
pubDatetime: 2026-06-08T04:10:05Z
modDatetime: 2026-06-08T04:10:05Z
...
tags:
  ...
  - oven
  - lg
most_likely_cause: "Upper convection fan failure or obstruction"
...
1. **Press CLEAR/OFF** and turn off the range at the circuit breaker for at least 30 seconds to reset the control board, then restore power and attempt a new preheat cycle.
2. **Confirm the symptom** by starting a preheat cycle with the door closed and using an oven thermometer to verify the cavity does not reach 150°F within five minutes.
3. **Inspect the upper convection fan** for mechanical seizure, obstructions, or damage. Disconnect power, remove the fan cover (usually inside the oven cavity or on the rear wall), and spin the fan blade by hand. Test the fan motor windings for continuity with a multimeter.
4. **Test the oven temperature sensor** by disconnecting it from the wiring harness and measuring its resistance at room temperature. Compare the reading to the resistance table in your model's service manual or wiring diagram.
5. **Check bake and broil element continuity** by removing each element's wire leads and measuring resistance across the terminals. A working element will show a low resistance (typically 15 to 50 ohms, depending on wattage). An open element will read infinite resistance.
6. **Inspect all wiring and connectors** between the elements, temperature sensor, convection fan, and the relay or main control board. Look for burnt spade terminals, melted insulation, or loose plugs. Repair or replace damaged harnesses.
7. **Test the relay control board output** if the fan, sensor, and elements all pass testing. Verify the board is sending voltage to the fan and element terminals during preheat. Replace the relay or main control board only after confirming all other components are functional.

## Parts Often Needed

| Part | Notes |
|------|-------|
| LG range upper convection fan assembly | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f7-error-code&k=LG+range+upper+convection+fan+assembly&tag=errorcodefixes-20) \| Verify the part number matches your exact LG range model. Fan assemblies are model-specific. |
| LG range oven temperature sensor (thermistor) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f7-error-code&k=LG+range+oven+temperature+sensor+%28thermistor%29&tag=errorcodefixes-20) \| Also called an oven sensor probe. Confirm compatibility with your model number before ordering. |
| LG range bake or broil element | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f7-error-code&k=LG+range+bake+or+broil+element&tag=errorcodefixes-20) \| Measure the failed element's resistance and compare to the service manual to confirm the correct wattage replacement. |
| LG range relay control board or main control board | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f7-error-code&k=LG+range+relay+control+board+or+main+control+board&tag=errorcodefixes-20) \| Only replace after all other components pass testing. Control boards are the most expensive part and the least likely cause of F7. |
