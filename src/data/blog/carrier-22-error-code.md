...
| Part | Notes |
|------|-------|
| Air filter | Most common fix; replace first |
| Blower run capacitor | Match µF and voltage exactly to OEM spec |
| High-limit switch | Verify OEM part number; temperature rating varies by model |
| Control board | Replace only if board relay is confirmed faulty |
