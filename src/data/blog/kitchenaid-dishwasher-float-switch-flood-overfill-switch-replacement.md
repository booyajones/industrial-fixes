---
title: "KitchenAid Dishwasher Float Switch Replacement - Signs & How-To"
description: "Dishwasher won't fill, stops filling too soon, or triggers overfill protection. Replacing the float switch or overfill control restores safe, normal fill operation."
...
- **Dishwasher will not fill at all** The control sees the float as already up (stuck closed switch) and refuses to start the fill cycle.
- **Fill stops too soon, leaving dishes dirty** The switch opens prematurely or the float is physically blocked partway up, cutting off water before the correct level is reached.
- **Overfill or flood protection fault code displayed** The control detects that the float/switch circuit is closed when it should be open, triggering a safety lockout.
- **Float visibly stuck, hard to move, or crusted with buildup** Soap scum, mineral deposits, or debris prevent the float from rising and falling freely in its housing.
- **No continuity change when float is pressed down** Testing the switch terminals with a multimeter shows the contacts do not change state when the float is actuated, confirming internal failure.
- **Water pooling in the base pan under the dishwasher** A leak has lifted the float or damaged the switch and wiring, and the overfill circuit is signaling a fault even after the leak is dried out.
...
| Part | Notes |
|------|-------|
| KitchenAid dishwasher float switch / overfill control switch | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-kitchenaid-dishwasher-float-switch-flood-overfill-switch-replacement&k=KitchenAid+dishwasher+float+switch+%2F+overfill+control+switch&tag=errorcodefixes-20) \| Common part numbers include W11545764, WPW10195039, WPW10102495, 8282619, and 8193506 (float switch and housing kit). Find your exact model and serial number on the label inside the door frame or on the side of the tub, then verify the correct replacement part number with a KitchenAid or Whirlpool parts supplier before ordering. |
