---
title: "Maytag Oven Won't Turn On - Causes & Fix"
description: "Usually a tripped breaker or blown fuse cutting power to the range. Reset the breaker or replace the fuse and verify the outlet supplies proper voltage."
pubDatetime: 2026-06-10T23:08:43Z
modDatetime: 2026-06-10T23:08:43Z
author: "Error Code Fixes Editorial Team"
featured: false
draft: false
tags:
  - appliance
  - oven
  - maytag
  - symptom
  - gas
---

## Maytag Oven Won't Turn On — What's Happening

When a Maytag oven won't turn on, the display is typically blank with no lights or response to button presses. In some cases the control may show a PF code after a power failure or an F9/F9 E0 code, which Maytag links to miswired or incorrect electrical connections at the outlet. On gas models the oven may appear completely dead or may power on but fail to ignite.

Maytag's own troubleshooting identifies the most common culprits as lost house power, tripped breakers or blown fuses, control board failures, incorrect electrical supply (especially on 240 V electric models), failed igniters on gas units, or less commonly demo mode activation or gas supply shutoffs.

## Most Likely Causes

- **Tripped breaker or blown fuse** The circuit breaker feeding the range has tripped or a fuse has blown, cutting all power to the oven and cooktop.
- **Miswired outlet or incorrect electrical connection** The 240 V receptacle is miswired or the range cord is not making proper contact, often triggering an F9 or F9 E0 code on the display.
- **Failed electronic control board** The main control board has failed or sustained visible damage, preventing the oven from powering on or accepting commands.
- **Power cord unplugged or loose** The range power cord is not fully seated in the wall receptacle or the terminal block connection inside the range is loose.
- **Failed igniter (gas models)** On gas ovens the igniter has burned out or cracked and cannot heat enough to open the gas valve, though this usually allows the control to power on.
- **Gas supply valve closed (gas models)** The main gas shutoff or regulator valve is closed, cutting fuel to the oven and preventing ignition.
- **Demo mode enabled** The oven is in showroom demo mode, which disables heating elements and igniters while the display remains active.

## How to Diagnose and Fix {#fix}

1. Verify the complaint by noting whether the display is completely blank, shows a PF code, or shows F9/F9 E0, and confirm whether you have an electric or gas model.
2. Check the house breaker panel and reset any tripped breakers feeding the range, or replace any blown fuses in older installations.
3. Inspect the power cord connection at the wall receptacle and verify it is fully seated and not damaged.
4. If the display shows F9 or F9 E0, do not assume an internal failure until a qualified electrician has verified the outlet is wired correctly and supplying proper voltage.
5. Turn off the breaker(s) feeding the range for one full minute, then restore power and observe whether the fault clears or returns.
6. Remove the rear access panel or control-panel cover and visually inspect the electronic control board for burn marks, swollen capacitors, or other obvious damage.
7. On gas models, confirm the main gas shutoff valve is open and listen for the igniter to glow and click when you attempt to start a bake cycle.
8. Check the Use and Care guide for your model to exit demo mode if the display is active but the oven will not heat.

## Parts You Might Need

| Part | Notes |
|------|-------|
| Electronic oven control board | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-maytag-oven-wont-turn-on&k=Electronic+oven+control+board&tag=errorcodefixes-20) \| Match the part number printed on your existing board or use your full model number. |
| Oven igniter (gas models) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-maytag-oven-wont-turn-on&k=Oven+igniter+%28gas+models%29&tag=errorcodefixes-20) \| Gas igniter work and gas-line diagnosis are best handled by a qualified technician. |
| Range power cord | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-maytag-oven-wont-turn-on&k=Range+power+cord&tag=errorcodefixes-20) \| Must match your outlet type (three-prong or four-prong) and voltage rating. |

## Related Error Codes

If your appliance also shows a code on the display, these match this problem:

- [Maytag Oven A6 error code](/posts/maytag-oven-a6-error-code/)
- [Maytag Oven Ab error code](/posts/maytag-oven-ab-error-code/)
- [Maytag Oven Cal error code](/posts/maytag-oven-cal-error-code/)
- [Maytag Oven F0 error code](/posts/maytag-oven-f0-error-code/)
- [Maytag Oven F1 error code](/posts/maytag-oven-f1-error-code/)
- [Maytag Oven F2 error code](/posts/maytag-oven-f2-error-code/)
- [Maytag Oven F3 error code](/posts/maytag-oven-f3-error-code/)
- [Maytag Oven F4 error code](/posts/maytag-oven-f4-error-code/)
- [Maytag Oven F5 error code](/posts/maytag-oven-f5-error-code/)
- [Maytag Oven F7 error code](/posts/maytag-oven-f7-error-code/)
- [Maytag Oven F8 error code](/posts/maytag-oven-f8-error-code/)
- [Maytag Oven F9 error code](/posts/maytag-oven-f9-error-code/)

## When to Call a Pro

Call a licensed electrician if F9 or F9 E0 persists after a power reset, if you are unsure about your outlet wiring, or if you do not have experience working with 240 V circuits. For gas models, have a qualified appliance or gas technician verify gas supply, inspect the igniter, and perform any repairs involving gas lines or valves. If the control board shows visible damage or the oven remains completely dead after verifying breaker and supply power, professional board-level diagnosis and replacement are recommended. For gas line, burner, or igniter work, or if you ever smell gas, stop and call a licensed technician.
