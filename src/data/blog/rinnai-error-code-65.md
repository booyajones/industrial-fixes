...
**Step 1 — Check inlet filter screen**
The most common fix for Error 65:
1. Turn off cold water supply to the unit
2. Disconnect the cold water inlet connection
3. Remove the filter screen (mesh disc inside the inlet)
4. Clean with water and a soft brush — replace if torn
...
| Part | Notes |
|---|---|
| Flow sensor (water flow rate sensor) | Rinnai part 100001226 or model-specific |
| Inlet filter screen | Rinnai part 100028 or equivalent |
| Flow control valve | Some models have a flow adjustment valve — may need cleaning |

## Reset Procedure

1. Fix the root cause (clean filter, restore flow)
2. Press the ON/OFF button to clear the error
3. Open a hot water fixture to verify the unit fires at normal flow
