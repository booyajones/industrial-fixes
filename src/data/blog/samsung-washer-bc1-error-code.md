...
- **Button physically stuck** Detergent residue, moisture, or a damaged bezel keeps the Power button or another key pressed down mechanically.
- **Power button held too long** Repeatedly pressing or holding the Power button during startup triggers the fault until the washer is power-cycled.
- **User-interface board fault** The button PCB or display board has failed internally and reads the key input as continuously active even when no button is pressed.
- **Wiring or connector issue** A loose or corroded connector between the button assembly and the main control board can cause intermittent or constant closed-circuit readings.
