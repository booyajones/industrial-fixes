...
- **Wrong speed-reference source selected** The drive parameter for frequency reference (keypad, analog voltage, analog current, or network) does not match the actual source feeding the signal.
- **Wrong run-command source selected** The drive parameter for run command (keypad, digital input terminals, or network) does not match the device sending the start signal.
- **Reference wiring on wrong terminals** A voltage reference signal is landed on A2 instead of A1, or a current reference is on A1 instead of A2, or the common wire is missing from AC.
- **DIP switch mismatch on control board** The control board DIP switches do not match the analog input type (voltage or current) configured in the parameters.
- **Open or loose control wiring** A wire in the analog reference circuit or run-command circuit is disconnected, broken, or making intermittent contact at a terminal.
- **Configuration not reinitialized after setup change** After a factory reset or parameter change, the drive was not stepped through the initial setup wizard to match the new wiring.
