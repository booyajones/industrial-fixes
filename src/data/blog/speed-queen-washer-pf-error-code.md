---
title: "Speed Queen Washer PF Error Code - Causes & Fix"
description: "PF means a power failure interrupted Delayed Start mode. Press Power/Cancel to clear the code and restart your washer normally."
pubDatetime: 2026-05-31T02:59:45Z
modDatetime: 2026-05-31T02:59:45Z
...
1. **Press Power/Cancel** to clear the PF code from the display and reset the control to idle mode.
2. **Check that the washer is plugged in firmly** and that the power cord has not been disturbed or pulled loose at the outlet.
3. **Verify the circuit breaker** for the washer is in the ON position and has not tripped.
4. **Test the wall outlet** with a multimeter or plug in another device to confirm the receptacle is delivering stable power.
5. **Power-cycle the washer if the code does not clear** by unplugging the machine for 30 seconds, then plugging it back in.
6. **Restart a new wash cycle** once the display is clear and the control has reset.
7. **Monitor for repeat PF codes** by noting whether the error returns during future delayed-start cycles, which would point to an unstable power supply or a control-retention issue.

## Parts Often Needed

| Part | Notes |
|------|-------|
| Power cord | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-speed-queen-washer-pf-error-code&k=Power+cord&tag=errorcodefixes-20) \| Replace if the cord jacket is damaged, the plug prongs are burned, or the cord does not seat securely in the outlet. |
| Wall receptacle / outlet | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-speed-queen-washer-pf-error-code&k=Wall+receptacle+%2F+outlet&tag=errorcodefixes-20) \| Install a new grounded outlet if the existing receptacle shows signs of arcing, loose contact, or cannot hold the plug firmly. |
| Main control board (front-end control) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-speed-queen-washer-pf-error-code&k=Main+control+board+%28front-end+control%29&tag=errorcodefixes-20) \| Required only if the washer will not clear PF after verified stable power or if the control falsely logs power-failure events. |
