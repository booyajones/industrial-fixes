...
- Dirty flame sensor
- Flame sensor wire issue
- Control board flame rectification fault
...
- **3 flashes**: pressure switch stuck closed or draft issue
- **4 flashes**: high-limit switch open
- **5 flashes**: flame sensed with no call for heat
- **Continuous rapid blinking**: polarity or grounding issue on some boards
...
| Furnace Filter (1-inch pleated) | High-limit trips from restricted airflow | [View on Amazon](https://www.amazon.com/s?i=industrial&k=1+inch+pleated+furnace+filter&tag=errorcodefixes-20) |
...
- The rollout switch is open
- The igniter tests good but never receives power
- The gas valve is not opening
- The pressure switch fault keeps coming back after vent and condensate checks
- The blower motor isn't starting
- The furnace breaker trips repeatedly
...
A click with no startup often means the thermostat relay closed but the furnace didn't complete the next step. If the control board has power, the most common causes are a stuck door switch, failed inducer motor, or bad igniter. Watch the startup sequence and note exactly where it stops.

**Why does my furnace start, then shut off after a few minutes?**
That usually points to airflow or limit issues. Start with the filter, all supply vents, and return grilles. If airflow is good, the blower motor may be weak or the high-limit switch may be opening because the heat exchanger is overheating.

**Can a dirty flame sensor really stop the whole furnace?**
Yes. The flame sensor is a safety device. If it can't prove flame within a few seconds, the control board shuts the gas valve immediately. A dirty flame sensor is one of the easiest and most common furnace fixes.

**What if the inducer runs but nothing else happens?**
That usually means the pressure switch didn't close, the igniter didn't energize, or the board is waiting on a safety input. Check the pressure switch tubing, condensate drain, flue blockage, and igniter resistance.

**Should I keep resetting the furnace if it locks out?**
No. A single reset after you check the filter and obvious issues is reasonable. Repeated resets without diagnosis just stress the igniter, inducer, and control board. If the lockout returns quickly, read the error code and troubleshoot the actual cause.
