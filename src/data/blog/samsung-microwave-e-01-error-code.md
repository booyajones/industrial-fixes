---
title: "Samsung Microwave E-01 Error - Causes & Fix"
description: "E-01 means Open Error on Samsung microwaves. Most often a door interlock switch or thermal safety is open. Power reset first, then check door."
pubDatetime: 2026-06-06T02:57:39Z
modDatetime: 2026-06-06T02:57:39Z
...
- **Open thermal protector or thermostat** A thermal fuse, thermal cutout, or cavity thermostat in the safety chain has opened permanently due to overheating or age and now appears as an open circuit.
...
1. **Disconnect power** by unplugging the microwave or switching off the dedicated circuit breaker and wait 60 seconds, then restore power to see if the E-01 code clears.
2. **Inspect the door system** by opening and closing the door several times, checking that the latch hook engages the striker cleanly, the hinges are not bent, and the door sits flush against the gasket with no gaps.
3. **Discharge the high-voltage capacitor** using an insulated screwdriver across its terminals (only if you are trained to do so) or call a technician, because the capacitor holds lethal voltage even when the unit is unplugged.
4. **Test door interlock switches** by locating the primary and secondary door switches behind the control panel, disconnecting each in turn, and using a multimeter to check continuity when the switch is actuated and open when released.
5. **Check thermal safety devices** by finding the thermal fuse or thermal cutout (often mounted on the magnetron or cavity side wall), removing it from the circuit, and testing continuity with the meter; a good thermal device shows near-zero ohms.
6. **Inspect wiring and connectors** along the harness from the door switches and thermal devices to the main control board, looking for broken wires, burned terminals, or connectors that have backed out or show corrosion.
7. **Replace the failed component** once you identify the open device (switch, thermal fuse, or connector), order the correct part by model number, install it, reassemble the cabinet, and power-cycle the microwave to verify the code does not return.

## Parts Often Needed

| Part | Notes |
|------|-------|
| Door interlock switch assembly | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-samsung-microwave-e-01-error-code&k=Door+interlock+switch+assembly&tag=errorcodefixes-20) \| Primary or secondary switch that closes when the door latches. Match your exact Samsung model number. |
| Thermal fuse or thermal cutout | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-samsung-microwave-e-01-error-code&k=Thermal+fuse+or+thermal+cutout&tag=errorcodefixes-20) \| One-time safety device that opens permanently if the cavity overheats. Check the amperage and mounting style for your model. |
| Main control board (PCB) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-samsung-microwave-e-01-error-code&k=Main+control+board+%28PCB%29&tag=errorcodefixes-20) \| Replace only after verifying all switches, safeties, and wiring test good. Order by the full model and serial number. |
| Wiring harness or connector kit | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-samsung-microwave-e-01-error-code&k=Wiring+harness+or+connector+kit&tag=errorcodefixes-20) \| If individual terminals or connectors are damaged. Some Samsung service kits include pre-crimped leads. |
