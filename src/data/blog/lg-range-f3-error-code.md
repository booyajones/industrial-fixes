---
title: "LG Range F3 Error Code - Causes & Fix"
description: "F3 on LG ranges means touch-pad key error or oven sensor short. Most often a dirty touch panel or failed sensor. Clean and reset first."
pubDatetime: 2026-06-08T04:07:30Z
modDatetime: 2026-06-08T04:07:30Z
...
1. **Press Clear or Off** on the control panel, then shut off the circuit breaker or unplug the range from the wall outlet.
2. **Wait one to two minutes** to allow the control board to fully discharge and clear any temporary fault memory.
3. **Restore power** and observe whether F3 reappears immediately or only during use. If the code does not return, monitor the range for a few cycles before taking further action.
4. **Inspect the touch panel** if the code returns. Wipe the entire control surface with a soft damp cloth to remove grease, moisture, or food residue, then dry it thoroughly.
5. **Check the oven sensor and its wiring** if your model documents F3 as a sensor fault. Pull the range forward, remove the rear access panel, and inspect the sensor connector at the back of the oven cavity for corrosion, loose pins, or damaged wires.
6. **Measure the oven sensor resistance** if you have a multimeter and your model's service literature. Disconnect the sensor, measure across its two terminals, and compare the reading to the specification in your wiring diagram. Field references often cite around 1080 to 1100 ohms at room temperature, but verify against your model's data before using that value as a pass or fail threshold.
7. **Replace the failed component** if the fault persists after cleaning, resetting, and testing. Install a new touch panel, oven sensor, wiring harness, or main control board depending on which part tested faulty, following the installation instructions in your service manual.

## Parts Often Needed

| Part | Notes |
|------|-------|
| LG range touch panel or keypad assembly | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f3-error-code&k=LG+range+touch+panel+or+keypad+assembly&tag=errorcodefixes-20) \| Match the part number on your existing panel or look up by model number on LG's parts site. |
| LG oven temperature sensor or thermistor | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f3-error-code&k=LG+oven+temperature+sensor+or+thermistor&tag=errorcodefixes-20) \| Usually a two-wire probe mounted in the rear wall of the oven cavity. Confirm the connector style before ordering. |
| LG range main control board or PCB | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-lg-range-f3-error-code&k=LG+range+main+control+board+or+PCB&tag=errorcodefixes-20) \| Only replace after confirming the touchpad and sensor circuits both test good. The board is the most expensive part in the repair path. |
