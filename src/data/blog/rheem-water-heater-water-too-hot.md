---
title: "Rheem Water Heater Water Too Hot - Causes & Fix"
description: "Usually a failed outlet temperature sensor or mineral scale buildup in a tankless unit (error code 16). Inspect the sensor and flush the heat exchanger."
pubDatetime: 2026-06-01T19:10:41Z
modDatetime: 2026-06-01T19:10:41Z
author: "James Rutherford"
featured: false
draft: false
tags:
  - water-heater
  - rheem
  - symptom
  - gas
---

## Rheem Water Heater Water Too Hot — What's Happening

When a Rheem water heater shows water too hot, it most commonly refers to an over-temperature fault on a tankless model, typically displayed as error code 16. Rheem describes this as outgoing water temperature being too hot or an outlet water temperature sensor problem. The fault can mean the controller is reading a falsely high temperature from a bad sensor, or the water is actually overheating due to low flow or scale buildup restricting heat transfer.

The exact meaning depends on your model. Tankless units trigger this fault when the outlet temperature exceeds safe operating limits or when the sensor circuit reports a reading outside normal range. Storage tank heaters use different fault codes and controls, so confirming your model type and any displayed error code is the first step.

[Jump to Fix](#fix)

## Most Likely Causes

- **Failed outlet temperature sensor or thermistor** The sensor may read falsely high or send inconsistent signals, causing the controller to think the water is overheating even when it is not.
- **Mineral scale or lime buildup in the heat exchanger** Scale restricts water flow and heat transfer, causing hot spots and inaccurate temperature readings that trigger the over-temperature fault.
- **Insufficient water flow through the unit** Low flow from closed valves, clogged inlet screens, flow restrictors, or low-flow fixtures can let the outlet temperature rise too quickly and trip the fault.
- **Loose or corroded wiring at the outlet sensor** Bad connections, corroded pins, or damaged harness wires at the sensor plug can create intermittent or out-of-range readings.
- **Control board misinterpreting sensor signal** Even with a good sensor and wiring, the PCB can report over-temperature due to internal fault or calibration drift, depending on the model family.
- **Excessive gas input or abnormal burner combustion** On gas tankless units, overfiring or incomplete combustion can cause real overheating, especially if gas pressure or burner settings are incorrect.

## How to Diagnose and Fix {#fix}

1. Verify the exact model number and any displayed error code on the control panel, since Rheem fault meanings vary by product line.
2. Measure the actual outlet water temperature with an external thermometer to confirm whether the water is truly too hot or the sensor is reporting incorrectly.
3. Check water flow rate and inlet conditions by inspecting for closed valves, clogged inlet filter or strainer, flow restrictors, or low-flow fixtures that could reduce flow below the minimum (typically 2 to 3 liters per minute on continuous-flow models).
4. Inspect the outlet temperature sensor and its wiring harness for loose connectors, corrosion, pin damage, or broken wires, and repair or replace any faulty connections.
5. Flush and descale the heat exchanger if you find evidence of mineral buildup or if the fault has recurred, following Rheem's recommended descaling procedure for your model.
6. Test the outlet temperature sensor resistance if your model manual provides a specification curve, and replace the sensor if readings are out of range or unstable.
7. For gas tankless units, have a licensed technician verify gas input pressure, burner operation, and combustion performance if the overheating appears real and flow and sensor checks are normal.
8. Replace only the confirmed failed component, then retest the unit under normal flow and temperature settings to verify the fault is cleared.

## Parts You Might Need

| Part | Notes |
|------|-------|
| Rheem outlet temperature sensor (thermistor) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-rheem-water-heater-water-too-hot&k=Rheem+outlet+temperature+sensor+%28thermistor%29&tag=errorcodefixes-20) \| Order by exact model number, sensor resistance varies by product family. |
| Descaling solution or vinegar flush kit | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-rheem-water-heater-water-too-hot&k=Descaling+solution+or+vinegar+flush+kit&tag=errorcodefixes-20) \| Use Rheem-approved descaler or white vinegar to remove mineral scale from the heat exchanger. |
| Inlet water filter or screen | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-rheem-water-heater-water-too-hot&k=Inlet+water+filter+or+screen&tag=errorcodefixes-20) \| Replace if clogged and restricting flow to the unit. |

## Related Error Codes

If your appliance also shows a code on the display, these match this problem:

- [Rheem Water Heater A01 error code](/posts/rheem-water-heater-a01-error-code/)
- [Rheem Water Heater Alarm 401 error code](/posts/rheem-water-heater-alarm-401-error-code/)

## When to Call a Pro

Call a licensed technician if you are not comfortable working with electrical sensors or wiring, if the fault persists after sensor replacement and descaling, or if gas burner or combustion diagnostics are needed. Gas pressure testing, burner adjustment, and control board replacement require specialized tools and safety knowledge. Professional help is also recommended if you cannot safely measure outlet temperature or flow rate, or if the unit repeatedly trips the over-temperature fault without an obvious cause. For gas line, burner, or igniter work, or if you ever smell gas, stop and call a licensed technician.

## See Also

- [Rheem RTEX Electric Tankless Water Heater Error Codes](/posts/rheem-rtex-electric-tankless-error-codes/)
- [Rheem Water Heater Thermocouple Replacement - Signs & How-To](/posts/rheem-water-heater-thermocouple-gas-pilot-sensor-replacement/)
- [Rheem Classic Series Furnace Error Codes — Complete Guide](/posts/rheem-classic-furnace-error-codes/)
- [Rheem Water Heater Heating Element Replacement - Signs & How-To](/posts/rheem-water-heater-heating-element-electric-screw-in-flange-replacement/)
