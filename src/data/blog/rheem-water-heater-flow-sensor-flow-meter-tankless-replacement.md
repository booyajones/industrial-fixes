---
title: "Rheem Tankless Water Heater Flow Sensor Replacement Guide"
description: "Error code 24, no hot water ignition, or intermittent shutdown point to flow sensor failure. Replacing the flow meter restores proper operation."
pubDatetime: 2026-06-01T21:37:47Z
modDatetime: 2026-06-01T21:37:47Z
author: "Marcus Webb"
featured: false
draft: false
tags:
...
- **Error code 24 on the display** Rheem troubleshooting documentation lists code 24 as a flow-sensor fault, meaning the board is not receiving the expected flow signal.
- **Unit does not ignite even with hot tap fully open** The control board blocks ignition because it does not detect the minimum flow rate required to fire the burner safely.
- **Intermittent shutdown during a shower or wash cycle** A failing sensor sends erratic flow readings, causing the heater to cycle off mid-demand even though water is still flowing.
- **Hot water works only at very high flow rates** A fouled sensor may register flow only when water velocity is much higher than the normal 2 to 3 liter-per-minute minimum.
- **No error code but heater never fires** If the flow sensor is completely dead or disconnected, the board receives no flow signal at all and will not permit burner operation.
- **Clicking or relay sound but no flame** You hear the ignition sequence start, but without a valid flow signal the gas valve remains closed and the burner does not light.
...
| Part | Notes |
|------|-------|
| Rheem tankless water heater flow sensor (flow meter) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-rheem-water-heater-flow-sensor-flow-meter-tankless-replacement&k=Rheem+tankless+water+heater+flow+sensor+%28flow+meter%29&tag=errorcodefixes-20) \| Part number varies by model. Check the data plate on the right side or inside the cabinet door for the full model and serial number, then search "Rheem [model] flow sensor" or call Rheem customer service with your serial to confirm the exact replacement part. |
| Flow sensor O-ring or gasket | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-rheem-water-heater-flow-sensor-flow-meter-tankless-replacement&k=Flow+sensor+O-ring+or+gasket&tag=errorcodefixes-20) \| Often included with the new sensor. If sold separately, verify thread size and material (typically Buna-N or silicone for hot-water service) against your model's parts diagram. |
