---
title: "GE Washer Main Control Board Replacement - Signs & How-To"
description: "No power, no lights, dead console, or washer won't start despite good AC power usually means a failed main control PCB. Replacing the board and programming it (if needed) restores operation."
pubDatetime: 2026-06-01T15:43:48Z
modDatetime: 2026-06-01T15:43:48Z
...
- **No status lights or indicator LEDs on the control board** When you open the console and the board itself shows no flashing or steady lights despite incoming 120 VAC, the main control board is considered bad.
- **Washer completely dead and will not power on** The console is blank, no display lights, and no beeps even though you have verified 120 VAC at the machine supply.
- **Console display is blank but board status lights are on** If the main board shows its normal status lights and you measure correct DC voltage (for example 13.8 VDC) at the board output but the interface still won't light up, suspect the user interface board or harness instead of the main PCB.
- **Washer powers on but will not start any cycle** The console may light up or beep, but pressing Start does nothing and no outputs (motor, valve, pump) respond, pointing to a failed main control PCB.
- **Intermittent operation or random shutdowns mid-cycle** The washer starts normally but drops out partway through, or cycles work one day and fail the next, often caused by cracked solder joints or failing relays on the board.
- **New board installed but washer still will not operate correctly** On some GE models the replacement main board must be programmed with the correct personality code or it will not run the machine properly.
...
| Part | Notes |
|------|-------|
| GE washer main control board / PCB | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-ge-washer-main-control-board-pcb-replacement&k=GE+washer+main+control+board+%2F+PCB&tag=errorcodefixes-20) \| Find your washer's model and serial number on the metal plate inside the door frame or on the rear panel, then cross-reference that model number with the correct board part number on the GE Appliances Parts site or your parts supplier. |
| GE washer user interface board (if diagnostics show the main board is good but the console will not respond) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-ge-washer-main-control-board-pcb-replacement&k=GE+washer+user+interface+board+%28if+diagnostics+show+the+main+board+is+good+but+the+console+will+not+respond%29&tag=errorcodefixes-20) \| Only replace the interface board if you have verified the main board is producing correct DC output (for example 13.8 VDC) and the harness is intact. |
