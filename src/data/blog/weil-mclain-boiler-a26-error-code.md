---
title: "Weil-McLain Boiler A26 Error - Causes & Fix"
description: "A26 error meaning varies by Weil-McLain model. Check your boiler's manual for the exact definition. Most often a sensor or limit switch fault."
pubDatetime: 2026-06-13T13:04:54Z
modDatetime: 2026-06-13T13:04:54Z
...
tags:
  - boiler
  - weil-mclain
money_part: "Temperature sensor (supply or return, model-specific)"
diy_or_pro: "pro"
free_checks:
  - "Find your boiler model and serial number (on the rating plate) and download the correct service manual from Weil-McLain's website."
  - "Check the control panel for additional error codes or fault history that may clarify which sensor or switch triggered A26."
...
1. **Identify your boiler model and control type.** Locate the rating plate (usually on the boiler jacket or inside the vestibule door) and note the exact model number and serial number.
2. **Retrieve the correct service manual.** Download the service instructions from Weil-McLain's website or contact their technical support to obtain the diagnostic table that defines A26 for your model.
3. **Check the control panel for additional fault codes.** Many Weil-McLain controls store a fault history or display multiple active codes. Write down every code shown.
4. **Power-cycle the boiler.** Turn off the boiler switch or circuit breaker for two minutes, then restore power. Observe whether A26 reappears immediately or only after a call for heat.
5. **Inspect all wiring and connectors.** With power off, check the terminals at the control board, all temperature and pressure sensors, limit switches, and the air-pressure switch for looseness, corrosion, or burn marks.
6. **Test the components named in the A26 diagnostic tree.** Using a multimeter, verify continuity or resistance of the sensor or switch identified by your manual. Replace only the failed component with the exact Weil-McLain part number.
7. **Restore power and monitor operation.** After replacing a sensor or switch, clear any stored fault codes (consult the manual for the reset procedure) and run a full heating cycle to confirm the error does not return.

## Parts Often Needed

| Part | Notes |
|------|-------|
| Temperature sensor (supply or return, model-specific) | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-weil-mclain-boiler-a26-error-code&k=Temperature+sensor+%28supply+or+return%2C+model-specific%29&tag=errorcodefixes-20) \| Order the exact OEM part number listed in your boiler's parts breakdown. Aftermarket sensors may have incorrect resistance curves. |
| Air pressure switch | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-weil-mclain-boiler-a26-error-code&k=Air+pressure+switch&tag=errorcodefixes-20) \| Verify the pressure-switch part number against your model's wiring diagram. Switches are calibrated to the blower and venting system. |
| High-limit control or aquastat | [Amazon](https://www.amazon.com/s?ascsubtag=ecf-weil-mclain-boiler-a26-error-code&k=High-limit+control+or+aquastat&tag=errorcodefixes-20) \| If the manual identifies A26 as a limit fault, replace the limit control with the manufacturer's specified part to maintain safety approvals. |
