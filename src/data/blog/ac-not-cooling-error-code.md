---
title: "Air Conditioner Not Cooling Error Codes Guide"
...
1. **Read the actual code or LED pattern** — Do not start with parts until you know whether the issue is pressure, temperature, communication, or power related.
2. **Check filter and blower airflow** — Poor indoor airflow is a fast way to create no-cooling complaints and freeze faults.
3. **Clean the outdoor coil** — Dirty condensers drive high head pressure and poor cooling performance.
4. **Verify fan and capacitor health** — Many no-cooling calls are really weak fan or compressor start issues.
5. **Use gauges only if qualified** — Pressure faults need real refrigerant measurements, not guesswork.
6. **Retest under load** — After a fix, let the unit run long enough to prove stable cooling.

## Parts and Tools Often Needed

| Item | Notes |
|------|-------|
| Filter | First check on any no-cooling call |
| Run capacitor | Frequent root cause on standard split systems |
| Contactor | Burned contacts cause compressor issues |
| Temperature sensor | Common on inverter and mini split gear |
| Coil cleaner | Needed for real condenser cleaning |
| Manifold gauges | Required for proper refrigerant diagnosis |
